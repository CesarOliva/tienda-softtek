                            ...
                            <div key={item.product_id} className="flex gap-3 text-sm">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <div className="mt-1 flex items-center gap-2">
                                        <button type="button" aria-label={`Quitar una unidad de ${item.nombre}`} onClick={() => removeItem(item.product_id)} className="rounded bg-neutral-800 p-1 hover:bg-neutral-700"><Minus className="size-3" /></button>
                                        <span className="min-w-4 text-center text-xs">{item.quantity}</span>
                                        <button type="button" aria-label={`Agregar una unidad de ${item.nombre}`} disabled={item.quantity >= item.stock} onClick={() => addItem(item)} className="rounded bg-neutral-800 p-1 hover:bg-neutral-700 disabled:cursor-not-allowed disabled:opacity-40"><Plus className="size-3" /></button>
                                        <button type="button" aria-label={`Eliminar ${item.nombre}`} onClick={() => removeItem(item.product_id)} className="ml-auto text-neutral-500 hover:text-white"><Trash2 className="size-3" /></button>
                                    </div>
                                </div>
                            </div>
                        ))}
                    </div>
                    <div className="mt-4 flex justify-between border-t border-neutral-800 pt-3 text-sm font-semibold">
                        <span>Total</span>
